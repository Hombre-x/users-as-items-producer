package warehouse.amenities

import cats.effect.std.Console
import cats.effect.{Async, Resource}
import cats.syntax.all.*
import fs2.io.net.Network
import fs2.kafka.{ProducerSettings, Serializer}
import org.typelevel.log4cats.Logger
import org.typelevel.otel4s.trace.Tracer
import skunk.*
import skunk.codec.text.text
import skunk.syntax.all.*
import warehouse.algebras.Producer
import warehouse.config.Config.AppConfig
import warehouse.domain.skunk.Pool
import warehouse.domain.commands.UserCommand
import warehouse.kafka.UserSerializer

import java.util.UUID

case class AppResources[F[_]](
    postgres: Pool[F],
    userProducer: Producer[F, UserCommand]
)

object AppResources:
  def make[F[_] : {Async, Tracer, Network, Logger, Console}](config: AppConfig): Resource[F, AppResources[F]] =

    def checkPostgresConnection(postgres: Pool[F]): F[Unit] = postgres.use: session =>
      session
        .unique(sql"select version();".query(text))
        .flatMap: v =>
          Logger[F].info(s"Connected to PostgreSQL $v")

    def mkPostgres: SessionPool[F] =
      Session
        .pooled[F](
          host = config.postgresConfig.host,
          port = config.postgresConfig.port,
          user = config.postgresConfig.user,
          password = Some(config.postgresConfig.password.value),
          database = config.postgresConfig.database,
          max = 8
        )
        .evalTap(checkPostgresConnection)

    def mkKafkaUserProducer: Resource[F, Producer[F, UserCommand]] =
      val settings =
        ProducerSettings[F, UUID, UserCommand](
          valueSerializer = UserSerializer.serializer[F],
          keySerializer = Serializer.uuid
        ).withBootstrapServers(config.producerConfig.bootstrapServer)

      Producer.kafka[F, UserCommand](settings, topic = "users-warehouse")

    (
      mkPostgres,
      mkKafkaUserProducer
    ).parMapN((postgres, userProducer) => AppResources(postgres, userProducer))
